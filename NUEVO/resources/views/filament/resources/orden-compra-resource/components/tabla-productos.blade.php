 ...
 --}}
                        <td class="col-unidad">
                            <select x-model="row.unidad" @focus="ensureUnidadesLoaded()"
                                ...
                                <option :value="row.unidad" x-show="row.unidad && !findUnidadOption(row.unidad)"
                                    x-text="row.unidad"></option>
    ...
    window.ordenCompraProductosTable = window.ordenCompraProductosTable || function(initialRows) {
        return {
            ...
            productoColumna(row) {
                ...
                const aux = String(row.descripcion_auxiliar ?? '').trim();
                const det = String(row.detalle_pedido ?? '').trim();
                const nombreProducto = String(row.producto ?? '').trim();

                const esAuxiliarConCodigo = !!String(row.codigo_visual ?? '').trim();
                const esAuxiliar = !!row.es_auxiliar || esAuxiliarConCodigo;

                if (!esAuxiliar) {
                    const label = nombreProducto || det || cod;
