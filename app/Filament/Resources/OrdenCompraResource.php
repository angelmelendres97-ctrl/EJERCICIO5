...
use Illuminate\Database\Eloquent\Model; // ESTA L脥NEA ES NECESARIA
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                /*
                |--------------------------------------------------------------------------
                | ESTE MODAL S脥 SE REACTIVA (Importar desde Pedido)
                |--------------------------------------------------------------------------
                */
                Forms\Components\Section::make('Informaci贸n Presupuesto')
                    ->headerActions([
                    ...
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('numero_factura_proforma')
                            ->label(fn(Get $get) => $get('formato') === 'P' ? 'N煤mero de proforma' : 'N煤mero de factura')
                            ->helperText('Ingrese el n煤mero seg煤n el formato seleccionado.')
                                    ...
                                    ->schema([
                                        ...
                                        Forms\Components\TextInput::make('detalle_pedido')
                                            ...
                                            ->afterStateHydrated(function (Set $set, Get $get, $state) {
                                                ...
                                                // Usa solo lo que exista (si no existe dped_desc_axiliar, quedar谩 null)
                                                ...
                                                if ($esAuxiliar) {
                                                    ...
                                                    $auxTxt = trim(collect([
                                                        $row->dped_cod_auxiliar ? 'C贸digo: ' . $row->dped_cod_auxiliar : null,
                                                        $descAux ? 'Nombre: ' . $descAux : null,
                                                    ])->filter()->implode(' | '));

                                                    $set('producto_auxiliar', $auxTxt ?: null);

                                                    // Tambi茅n deja listo el JSON "detalle" si lo usas en otras partes
                                                ...
                                                if ($esServicio) {
                                                    $srvTxt = trim(collect([
                                                        $codigoProd ? 'C贸digo servicio: ' . $codigoProd : null,
                                                        $detallePedido ? 'Descripci贸n: ' . $detallePedido : null,
                                                    ])->filter()->implode(' | '));
